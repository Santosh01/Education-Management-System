...
Partial Class mstSubject
    ...
    Protected Sub Page_Load1(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            subjectcode.ToolTip = "Subject Code"
            subjectname.ToolTip = "Subject Name"
    ...
    Protected Sub AutoNumberNo()
        ...
        Try
            ...
            con.Open()
            com = New SqlCommand("SELECT MAX(subjectcode) AS Max_Number FROM  subjectmaster ", con)
            NextNo = com.ExecuteScalar() + 1
            con.Close()
            subjectcode.Text = NextNo.ToString()
    ...
    Protected Sub Btnsave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnsave.Click
        ...
        Try
            ...
            Dim com As New SqlCommand("SELECT * FROM subjectmaster WHERE syssubjectno =0" & syssubjectno.Text, con)
            ...
            End If
            dr("syssubjectno") = Val(syssubjectno.Text)
            dr("subjectcode") = subjectcode.Text
            dr("subjectname") = subjectname.Text
    ...
    Protected Sub btnEdit_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            syssubjectno.Text = row.Cells(0).Text

            Dim ds As New DataSet
            ds = DataSetCreator.GetDataSet("select * from subjectmaster where syssubjectno =0" & syssubjectno.Text)

            syssubjectno.Text = ds.Tables(0).Rows(0)("syssubjectno")
            subjectcode.Text = ds.Tables(0).Rows(0)("subjectcode")
            subjectname.Text = ds.Tables(0).Rows(0)("subjectname")
        Catch ex As Exception
            DisplayCustomMessageSummary(ex.Message)
        End Try
    End Sub
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            Dim row As GridViewRow = DirectCast(update.NamingContainer, GridViewRow)
            syssubjectno.Text = row.Cells(0).Text
            DataSetCreator.ExecuteQuery("UPDATE subjectmaster SET deleted = 'Y' WHERE syssubjectno =0" & syssubjectno.Text)
            RefreshGrid()
        Catch ex As Exception
            DisplayCustomMessageSummary(ex.Message)
        End Try
    ...
    Private Sub RefreshGrid()
        gvList.DataSource = DataSetCreator.GetDataSet("SELECT * FROM subjectmaster WHERE deleted = 'N' ORDER BY subjectcode ")
        gvList.DataBind()
    End Sub
